 ...
 * @returns {Promise<any>} - The updated folder document.
 */
export const editFolder = async (id: string, data: Partial<IFolder>): Promise<any> => {
    const result = await FolderSchema.findOneAndUpdate({ _id: id }, data);
    return result;
};
 ...
 */
export const getFoldersByParentId = async (parentId: string): Promise<IFolder[]> => {
...
export const findFolderByName = async (name: string): Promise<IFolder | null> => {
    try {
